...
export default async function CoachAthleteView({
  ...
  const [coach, coachAthlete, weekly, risk] = await Promise.all([
    getAthlete(COACH_ID),
  ...
  const useMetric = prefersMetric(coach.measurement_preference);
          ...
          <tbody>
            {(weekly.data ?? []).map((w) => (
              <tr key={w.week_start} className="border-t">
                ...
                <td className="py-2 text-right font-mono tabular-nums">
                  {distanceFn(w.run_distance_meters, 1)} {unit}
                </td>
                <td className="py-2 text-right font-mono tabular-nums">
